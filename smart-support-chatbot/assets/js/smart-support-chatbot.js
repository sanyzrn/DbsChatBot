...
( function () {
        ...
        function parseResponse( r ) {
                return r.text().then( function ( txt ) {
                        ...
                        if ( json && ! r.ok && json.message && ! json.data ) {
                                json = { success: false, code: json.code, data: { message: json.message } };
        ...
        function ajax( action, data ) {
                ...
                return fetch( cfg.restUrl + route, {
                        ...
                        // اگر سایت nonce را سخت‌گیرانه الزام کرده و صفحه cacheشده nonce
                        // منقضی فرستاده، یک‌بار مسیر قدیمی را امتحان می‌کنیم تا کاربر
                        // به‌جای خطای «نشست منقضی شده» فقط یک retry پنهان ببیند.
                        if ( 403 === r.status ) {
                                return parseResponse( r ).then( function ( res ) {
                                        var code = res.json && res.json.code ? res.json.code : '';
                                        if ( 'ssc_bad_nonce' === code ) {
                                                return ajaxLegacy( action, data );
                                        }
                                        return res;
                                } );
        ...
        function submitForm() {
                ...
                var payload = {
                        ...
                        // رضایت سمت سرور هم اعتبارسنجی می‌شود؛ مقدار ارسال می‌شود تا
                        // وقتی الزامی است، کلاینت‌های مستقیم نتوانند آن را دور بزنند.
                        consent: ( cfg.consentEnabled && state.form.consent ) ? 'true' : 'false'
                };
                if ( isAdr ) {
