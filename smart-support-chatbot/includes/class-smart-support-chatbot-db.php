...
class SSC_Chatbot_DB {
         ...
         * روی هر درخواست فراخوانی می‌شود (هوک init)، پس ابتدا با یک get_option ارزان بود/نبودِ
         * نیاز به مهاجرت را چک می‌کنیم و در صورت نیاز با قفل گذرا از اجرای هم‌زمانِ
         * دو درخواست جلوگیری می‌شود.
         */
        public static function maybe_upgrade() {
                ...
                // قفل گذرا: اگر درخواست دیگری همین الان در حال مهاجرت است، منتظر نمی‌مانیم؛
                // dbDelta ذاتاً idempotent است و درخواست بعدی هم می‌تواند آن را تکرار کند،
                // اما برای کاهش بار، اجرای موازی را با قفل ۳۰ ثانیه‌ای محدود می‌کنیم.
                if ( get_transient( 'ssc_chatbot_upgrade_lock' ) ) {
                        return;
                }
                set_transient( 'ssc_chatbot_upgrade_lock', 1, 30 );
         ...
         * پشتیبانی از صفحه‌بندی (batch) تا خروجی جدول بزرگ کل حافظه را اشغال نکند.
         *
         * @param array $args   فیلترها.
         * @param int   $limit  حداکثر تعداد ردیف در هر فراخوانی (۰ = بدون محدودیت).
         * @param int   $offset آفست (برای خواندن دسته‌ای).
         * @return array
         */
        public static function get_filtered_for_export( $args, $limit = 0, $offset = 0 ) {
                global $wpdb;
                $table = self::table_name();
                $args  = wp_parse_args(
                        $args,
                ...
                if ( $limit > 0 ) {
                        ...
                        $params = array_merge( $params, array( (int) $limit, (int) $offset ) );
                }
                // phpcs:ignore WordPress.DB
        ...
        public static function stat_bump( $metric, $n = 1, $date = '' ) {
                ...
                $n     = (int) $n;
                // ستون metric از نوع VARCHAR(80) است؛ متریک بلندتر (مثلاً product:{شناسهٔ بلند})
                // باعث شکست خاموش INSERT می‌شود، پس کوتاه می‌شود.
                $metric = mb_substr( (string) $metric, 0, 80 );
                // phpcs:ignore WordPress.DB
                $wpdb->query(
                        $wpdb->prepare(
         ...
         * معیار جدیت: شدتِ جدی «یا» پیامدِ جدی (بستری/فوت/ناتوانی). پیش‌تر فقط ستون
         * severity بررسی می‌شد و گزارشی با شدت «متوسط» و پیامد «فوت» اصلاً شمرده نمی‌شد.
         *
         * @param array $severities فهرست شدت‌های جدی.
         * @param array $outcomes   فهرست پیامدهای جدی (اختیاری).
         * @return int
         */
        public static function serious_adr_count( $severities, $outcomes = array() ) {
                $severities = array_values( array_filter( (array) $severities ) );
                $outcomes   = array_values( array_filter( (array) $outcomes ) );
                if ( empty( $severities ) && empty( $outcomes ) ) {
                        return 0;
                }
                global $wpdb;
                $table = self::table_name();
                ...
                $params = array();
                if ( ! empty( $severities ) ) {
                        $conds[] = 'severity IN ( ' . implode( ', ', array_fill( 0, count( $severities ), '%s' ) ) . ' )';
                        $params  = array_merge( $params, $severities );
                }
                if ( ! empty( $outcomes ) ) {
                        $conds[] = 'outcome IN ( ' . implode( ', ', array_fill( 0, count( $outcomes ), '%s' ) ) . ' )';
