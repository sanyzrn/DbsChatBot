...
class SSC_Chatbot_Admin {
        ...
        public function render_dashboard_page() {
                ...
                $insights     = array(
                        ...
                        'serious'    => SSC_Chatbot_DB::serious_adr_count(
                                apply_filters( 'ssc_chatbot_serious_severities', array( 'شدید', 'تهدیدکننده حیات' ) ),
                                apply_filters( 'ssc_chatbot_serious_outcomes', array( 'منجر به بستری شد', 'فوت', 'عارضه ماندگار/ناتوانی' ) )
                        ),
                        'qa_count'   => SSC_Chatbot_DB::qa_count(),
                        'csat'       => SSC_Chatbot_DB::get_csat_stats(),
                );
                // آخرین خطای اعلان (بله/تلگرام/ایمیل) — شکست اعلان دیگر بی‌صدا نماند.
                $notify_error = get_transient( 'ssc_chatbot_notify_error' );
        ...
        public function handle_actions() {
                ...
                if ( isset( $_POST['ssc_chatbot_save_qa'] ) ) {
                        ...
                        // پیام هشدار (مثل بریده‌شدن POST با max_input_vars) با استایل هشدار نمایش داده شود، نه موفقیت.
                        $notice_class = ( 0 === strpos( $msg, '⚠️' ) ) ? 'notice-warning' : 'notice-success';
                        add_action(
                                'admin_notices',
                                function () use ( $msg, $notice_class ) {
                                        echo '<div class="notice ' . esc_attr( $notice_class ) . ' is-dismissible"><p>' . esc_html( $msg ) . '</p></div>';
                                }
                        );
                }
        ...
        protected function parse_qa_import( $content, $fname ) {
                ...
                $first = true;
                // پارامتر escape صریح (سازگاری PHP 8.4+).
                ...
                while ( false !== ( $cols = fgetcsv( $stream, null, ',', '"', '\\' ) ) ) {
                        // ردیف خالی.
        ...
        /**
         * ساخت شناسهٔ پایدار محصول/فیلد — به helper مشترک در Settings منتقل شد تا
         * مسیر ویجت المنتور هم (که کلاس Admin را ندارد) از همان منطق استفاده کند.
         *
         * @param string $raw_id ورودی خام شناسه.
         * @param string $name   نام نمایشی.
         * @param array  $taken  شناسه‌های استفاده‌شده.
         * @return string
         */
        protected static function make_product_id( $raw_id, $name, $taken ) {
                return SSC_Chatbot_Settings::make_unique_id( $raw_id, $name, $taken );
        }
        ...
        protected function save_settings() {
                ...
                // محصولات + دانش محصولات.
                // باگ پیش‌تر: وقتی مدیر «همهٔ» محصولات را حذف می‌کرد، مقدار قبلی در option
                // باقی می‌ماند (چون خالی‌ها ذخیره نمی‌شدند) و محصول حذف‌شده در چت ظاهر می‌شد.
                // اکنون همیشه از فرم بازخوانی می‌شود؛ فهرست خالی = حذف کامل.
                $products      = array();
                $knowledge_map = array();
                ...
                // پاسخ‌های پیشنهادی — همیشه ذخیره می‌شود؛ خالی کردن همهٔ ردیف‌ها باید واقعاً خالی ذخیره شود.
        ...
        public function export_csv() {
                ...
                // پارامتر escape صریح است تا در PHP 8.4+ هشدار deprecation نگیرد و رفتار ثابت بماند.
                // (\ همان مقدار پیش‌فرض تاریخی PHP است — فقط برای سکوت کردن هشدار صراحت می‌شود.)
                $csv_args = array( ',', '"', '\\' );
                ...
                // برچسب‌های متادیتای رضایت (در extra_fields ذخیره می‌شوند).
                $field_labels['_consent']       = __( 'زمان ثبت موافقت با حریم خصوصی', 'smart-support-chatbot' );
                $field_labels['_consent_hash']  = __( 'هش متن رضایت', 'smart-support-chatbot' );
                $field_labels['_consent_policy'] = __( 'آدرس سیاست حریم خصوصی', 'smart-support-chatbot' );

                // خواندن دسته‌ای (batch) برای جلوگیری از اشغال حافظه متناسب با کل جدول.
                $batch_size = (int) apply_filters( 'ssc_chatbot_export_batch_size', 500 );
                $offset     = 0;
                do {
                        $rows = $has_filter
                                ? SSC_Chatbot_DB::get_filtered_for_export( $filters, $batch_size, $offset )
                                : SSC_Chatbot_DB::get_filtered_for_export( array(), $batch_size, $offset );
                        foreach ( $rows as $r ) {
                                $extra_txt = '';
                                ...
                                fputcsv( $out, array_map( array( __CLASS__, 'csv_safe' ), $cells ), ...$csv_args );
                        }
                        $offset += $batch_size;
