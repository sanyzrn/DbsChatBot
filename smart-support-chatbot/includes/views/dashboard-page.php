...
<div class="wrap ssc-admin" dir="rtl">
        ...
        <?php if ( ! empty( $notify_error ) && is_array( $notify_error ) ) : ?>
                <div class="notice notice-warning">
                        <p>
                                <strong><?php esc_html_e( '⚠️ شکست در ارسال اعلان:', 'smart-support-chatbot' ); ?></strong>
                                <?php
                                $ne_channel = isset( $notify_error['channel'] ) ? (string) $notify_error['channel'] : '';
                                $ne_error   = isset( $notify_error['error'] ) ? (string) $notify_error['error'] : '';
                                $ne_at      = isset( $notify_error['at'] ) ? (int) $notify_error['at'] : 0;
                                /* translators: 1: کانال (بله/تلگرام/ایمیل), 2: پیام خطا, 3: زمان */
                                echo esc_html( sprintf( __( 'ارسال از طریق «%1$s» ناموفق بود: %2$s — (%3$s)', 'smart-support-chatbot' ), $ne_channel, $ne_error, date_i18n( 'Y/m/d H:i', $ne_at ) ) );
                                ?>
                                <?php esc_html_e( 'تنظیمات اعلان یا اتصال سرور را بررسی کنید. با نخستین ارسال موفق، این هشدار حذف می‌شود.', 'smart-support-chatbot' ); ?>
                        </p>
                </div>
        <?php endif; ?>
