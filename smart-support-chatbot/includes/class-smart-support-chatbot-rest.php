...
class SSC_Chatbot_REST {
        ...
        /**
         * بررسی دسترسی: nonce استاندارد REST وردپرس — فقط در حالت سخت‌گیرانه.
         *
         * پیش‌فرض: nonce الزامی نیست. دلیل: nonce از HTML صفحه می‌آید و با page-cache
         * (LiteSpeed، WP Rocket، CDN) منقضی می‌شود؛ چت‌بات روی سایت‌های cacheشده
         * ساعت‌ها ۴۰۳ می‌داد و با کلاینت‌های headless/موبایل هم کار نمی‌کرد.
         * این endpointها «عمومی»‌اند (مهمان چت می‌کند) و nonce برای مهمان محافظت
         * واقعی نیست؛ دفاع واقعی: محدودیت نرخ اتمیک، honeypot، سقف طول پیام،
         * اعتبارسنجی محصول/نوع، و سقف IP پشتیبان.
         * برای سایت‌هایی که با این توافق مخالف‌اند، فیلتر ssc_chatbot_enforce_rest_nonce
         * را true کنید تا رفتار سخت‌گیرانهٔ قبلی برگردد.
         *
         * @param WP_REST_Request $request درخواست.
         * @return true|WP_Error
         */
        public function check_nonce( $request ) {
                if ( ! apply_filters( 'ssc_chatbot_enforce_rest_nonce', false ) ) {
                        return true;
                }
