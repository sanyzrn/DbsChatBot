...
class SSC_Chatbot_Ajax {
        ...
        /**
         * اعتبارسنجی nonce برای endpointهای عمومی چت (سازگار با page-cache).
         *
         * چرا سخت‌گیری نکردیم؟ nonce از خودِ HTML صفحه تولید می‌شود؛ وقتی سایت از
         * page-cache (مثل LiteSpeed/WP Rocket/CDN) استفاده کند، صفحهٔ cacheشده حاوی
         * nonce منقضی است و با بررسی اجباری، چت برای بازدیدکننده‌ها ساعت‌ها ۴۰۳ می‌دهد.
         * برای مهمان‌ها nonce هیچ محافظت واقعی‌ای هم ندارد (نشست احرازشده‌ای در کار نیست)
         * و دفاع اصلی محدودیت نرخ + honeypot + سقف طول پیام است. پس:
         *  - اگر nonce معتبر باشد: پذیرفته می‌شود.
         *  - اگر نباشد/منقضی باشد و کاربر مهمان باشد: پذیرفته می‌شود (رفع باگ کش).
         *  - برای کاربر لاگین‌شده: nonce معتبر الزامی است (محافظت CSRF معنا دارد).
         *
         * @return bool
         */
        protected function verify_public_nonce() {
                $nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['nonce'] ) ) : '';
                if ( $nonce && wp_verify_nonce( $nonce, 'ssc_chatbot_nonce' ) ) {
                        return true;
                }
        ...
        protected function check_rate_limit( $bucket ) {
                ...
                if ( 'off' === $mode ) {
                        // فقط برای csat (سایر سطل‌ها بالا return شده‌اند): با محدودسازیِ خاموش،
                        // میانگین رضایت همچنان نباید با رأی انبوه جعل‌پذیر باشد؛
                        // یک سقف سخاوتمندانهٔ روزانه روی IP همیشه اعمال می‌شود.
                        $checks[] = array(
        ...
        public function handle_chat() {
                if ( ! $this->verify_public_nonce() ) {
                        wp_send_json_error( array( 'message' => __( 'نشست شما منقضی شده است. لطفاً صفحه را تازه‌سازی کنید.', 'smart-support-chatbot' ) ), 403 );
                }
        ...
        protected function generate_ai_reply( $message, $product_id, $history = array() ) {
                ...
                if ( $use_ai ) {
                        ...
                        $cache_key     = '';
                        if ( $cache_enabled ) {
                                // اثر انگشت تنظیمات مؤثر هم در کلید کش؛ پیش‌تر تغییر مدل/temperature/max_tokens
                                // تا پایان TTL پاسخِ تنظیمِ قبلی برمی‌گشت.
                                $ai_fingerprint = md5(
                                        implode(
                                                '|',
                                                array(
                                                        SSC_Chatbot_Settings::get( 'gemini_model', '' ),
                                                        SSC_Chatbot_Settings::get( 'openai_model', '' ),
                                                        SSC_Chatbot_Settings::get( 'claude_model', '' ),
                                                        SSC_Chatbot_Settings::get( 'custom_model', '' ),
                                                        SSC_Chatbot_Settings::get( 'ai_temperature', '0.4' ),
                                                        SSC_Chatbot_Settings::get( 'ai_max_tokens', 800 ),
                                                )
                                        )
                                );
                                $cache_key = 'ssc_ai_' . md5( $provider . '|' . $product_id . '|' . mb_strtolower( trim( $message ) ) . '|' . $ai_fingerprint . '|' . md5( $system ) );
        ...
        protected function dispatch_ai( $provider, $message, $product_id, $product_name, $system, $messages, $history ) {
                switch ( $provider ) {
                        ...
                        case 'custom':
                                $custom_endpoint = SSC_Chatbot_Settings::get( 'custom_endpoint', '' );
                                // endpoint سفارشی کلید API حمل می‌کند؛ باید عمومی و HTTPS باشد (ضد SSRF / ضد افشای کلید).
                                if ( $custom_endpoint && ! $this->is_safe_endpoint( $custom_endpoint, true ) ) {
                                        $this->last_error = 'آدرس endpoint سفارشی مجاز نیست (باید آدرسی عمومی با HTTPS باشد).';
                                        return '';
                                }
                                return $this->openai_compatible_reply(
                                        $custom_endpoint,
        ...
        /**
         * اعتبارسنجی آدرس HTTP خروجی (ضد SSRF).
         *
         * endpointهای سفارشی با wp_remote_post فراخوانی می‌شدند؛ مدیرِ به‌خطرافتاده (یا
         * ورودی اشتباه) می‌توانست سرور را وادار به درخواست به آدرس‌های داخلی
         * (127.0.0.1، 169.254.169.254، 10.x، …) یا ارسال کلید API روی HTTP کند.
         * این helper فقط آدرس عمومیِ معتبر می‌پذیرد و برای endpoint دارای credential،
         * HTTPS را الزامی می‌کند (با فیلتر برای نصب‌های خاص).
         *
         * @param string $url          آدرس.
         * @param bool   $needs_https  آیا HTTPS الزامی است (endpoint دارای کلید/امضا).
         * @return bool
         */
        protected function is_safe_endpoint( $url, $needs_https = false ) {
                $url = trim( (string) $url );
                ...
                // اگر ورودی مستقیماً IP است، همین‌جا رد می‌شود اگر خصوصی باشد.
                if ( filter_var( $host, FILTER_VALIDATE_IP ) ) {
                        $ip = $host;
                } else {
                        // واکشی DNS برای جلوگیری از دورزدن با نام دامنه‌ای که به IP داخلی اشاره می‌کند.
                        $ip = gethostbyname( $host );
                        if ( $ip === $host ) {
                                return false; // نام حل نشد.
                        }
                }
                ...
                if ( $needs_https && 'https' !== strtolower( wp_parse_url( $url, PHP_URL_SCHEME ) ) ) {
                        return false;
                }
                return true;
        }
        ...
        protected function webhook_reply( $message, $product_id, $product_name, $history = array() ) {
                ...
                // با تنظیم secret، آدرس باید HTTPS باشد (امضا روی HTTP قابل شنود است).
                if ( ! $this->is_safe_endpoint( $url, (bool) $secret ) ) {
                        $this->last_error = 'آدرس Webhook مجاز نیست (باید آدرسی عمومی و معتبر باشد و در صورت تنظیم secret، از HTTPS استفاده کند).';
                        return '';
                }
                ...
                // اعتبارسنجی امضای پاسخ — وقتی secret تنظیم شده، نبودِ هدر امضا هم رد می‌شود
                // (fail-closed). رفتار قبلی «اختیاری با سکوت» بود؛ پاسخ دستکاری‌شده بی‌صدا قبول می‌شد.
                if ( $secret ) {
                        $resp_sig = wp_remote_retrieve_header( $response, 'x-ssc-signature' );
                        $expected = 'sha256=' . hash_hmac( 'sha256', $body, $secret );
                        if ( ! $resp_sig || ! hash_equals( $expected, (string) $resp_sig ) ) {
                                $this->last_error = 'امضای پاسخ Webhook نامعتبر است یا غایب است.';
                                return '';
                        }
                }
        ...
        public function submit( $args ) {
                ...
                // الزام رضایت حریم خصوصی سمتِ سرور. پیش‌تر این کنترل فقط در جاوااسکریپت
                // بود و هر کلاینتِ مستقیم (REST/AJAX) می‌توانست بدون رضایت، دادهٔ شخصی ثبت کند.
                $consent_required = ( 'yes' === SSC_Chatbot_Settings::get( 'consent_enabled', 'no' ) );
                $consent_given    = ! empty( $args['consent'] ) && ! in_array( (string) $args['consent'], array( 'false', '0', 'no', 'off' ), true );
                if ( $consent_required && ! $consent_given ) {
                        return new WP_Error(
                                'ssc_consent_required',
                                __( 'برای ثبت این فرم، موافقت با حریم خصوصی الزامی است.', 'smart-support-chatbot' ),
                                array( 'status' => 400 )
                        );
                }
                if ( ! $consent_required ) {
                        // اگر رضایت از تنظیمات خاموش شده باشد، نباید فیلد تقلبی ثبت شود.
                        $consent_given = false;
                }
                ...
                // قفل سمت سرور روی قابلیت «ثبت عوارض»: اگر مدیر گزینه را در تنظیمات خاموش کرده،
                // هیچ مسیر مستقیمی نباید بتواند فیلدهای ADR را ثبت کند یا نوع فرم را عوارضی جلوه دهد.
                $adr_allowed = ( 'yes' === SSC_Chatbot_Settings::get( 'show_adr', 'no' ) );
                if ( $is_adr && ! $adr_allowed ) {
                        $is_adr = false;
                        $type   = __( 'درخواست مشاوره', 'smart-support-chatbot' );
                }
                ...
                // ثبت سند رضایت (مدرک audit): زمان، آدرس سیاست و هشِ متن رضایت نمایش‌داده‌شده.
                // در extra_fields ذخیره می‌شود تا بدون تغییر ساختار جدول، مدرک ماندگار باشد.
                if ( $consent_given ) {
                        $consent_meta = array(
                                '_consent' => current_time( 'mysql' ),
                        );
                        if ( '' !== (string) SSC_Chatbot_Settings::get( 'consent_link', '' ) ) {
                                $consent_meta['_consent_policy'] = SSC_Chatbot_Settings::get( 'consent_link', '' );
                        }
                        $consent_text = (string) SSC_Chatbot_Settings::get( 'consent_text', '' );
                        if ( '' !== $consent_text ) {
                                $consent_meta['_consent_hash'] = substr( md5( $consent_text ), 0, 12 );
                        }
                        $row['extra_fields'] = wp_json_encode( is_array( $extra ) ? array_merge( $extra, $consent_meta ) : $consent_meta );
                }
        ...
        /**
         * آیا گزارش عارضه «جدی» است؟
         *
         * پیش‌تر فقط شدت بررسی می‌شد؛ در نتیجه گزارشی با شدت «متوسط» و پیامد «فوت» یا
         * «منجر به بستری شد» هشدار فوری نمی‌گرفت و در شمارندهٔ جدی داشبورد هم نمی‌آمد.
         * طبق تعریف استاندارد جدیت (ICH E2A)، پیامدهای بستری/ناتوانی/فوت هم جدی‌اند.
         *
         * @param array $row داده‌ها.
         * @return bool
         */
        protected function is_serious_adr( $row ) {
                $serious_severities = apply_filters( 'ssc_chatbot_serious_severities', array( 'شدید', 'تهدیدکننده حیات' ) );
                $serious_outcomes   = apply_filters( 'ssc_chatbot_serious_outcomes', array( 'منجر به بستری شد', 'فوت', 'عارضه ماندگار/ناتوانی' ) );
                if ( ! empty( $row['severity'] ) && in_array( $row['severity'], (array) $serious_severities, true ) ) {
                        return true;
                }
                return ! empty( $row['outcome'] ) && in_array( $row['outcome'], (array) $serious_outcomes, true );
        }
         ...
         *
         * شکست اعلان (توکن اشتباه، قطعی شبکه، مسدودسازی) دیگر بی‌صدا نادیده گرفته نمی‌شود:
         * ثبت در error_log، رویداد برای توسعه‌دهندگان و نگهداری آخرین خطا برای نمایش در داشبورد.
         *
         * @param array $row داده‌ها.
         */
        protected function maybe_send_messenger_notification( $row ) {
                ...
                if ( is_wp_error( $response ) ) {
                        ...
                        $code = (int) wp_remote_retrieve_response_code( $response );
                        if ( 200 !== $code ) {
                                $err = 'HTTP ' . $code . ' — ' . mb_substr( wp_strip_all_tags( wp_remote_retrieve_body( $response ) ), 0, 200 );
                        }
                }
                if ( '' !== $err ) {
                        // ثبت شکست برای عیب‌یابی و نمایش در داشبورد + رویداد برای توسعه‌دهندگان.
                        error_log( '[SSC Chatbot] Notification (' . $platform . ') failed: ' . $err ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
                        set_transient( 'ssc_chatbot_notify_error', array( 'channel' => $platform, 'error' => $err, 'at' => time() ), WEEK_IN_SECONDS );
                        do_action( 'ssc_chatbot_notification_failed', $platform, $row, $err );
                        return;
                }
                delete_transient( 'ssc_chatbot_notify_error' );
                do_action( 'ssc_chatbot_notification_sent', $platform, $row );
        }

        /**
         * ارسال اعلان ایمیلی.
         *
         * نتیجهٔ wp_mail بررسی می‌شود؛ شکست دیگر بی‌صدا نادیده گرفته نمی‌شود.
         *
         * @param array $row داده‌ها.
         */
        protected function maybe_send_email_notification( $row ) {
                ...
                /* translators: %s: نوع فرم. */
                $subject = sprintf( __( 'درخواست جدید: %s', 'smart-support-chatbot' ), $row['type'] );
                $sent    = wp_mail( $to, $subject, $this->build_notification_text( $row ) );
                if ( ! $sent ) {
                        error_log( '[SSC Chatbot] Email notification failed (to: ' . $to . ').' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
                        set_transient( 'ssc_chatbot_notify_error', array( 'channel' => 'email', 'error' => 'wp_mail returned false (to: ' . $to . ')', 'at' => time() ), WEEK_IN_SECONDS );
                        do_action( 'ssc_chatbot_notification_failed', 'email', $row, 'wp_mail returned false' );
                        return;
                }
                delete_transient( 'ssc_chatbot_notify_error' );
                do_action( 'ssc_chatbot_notification_sent', 'email', $row );
        }
}
