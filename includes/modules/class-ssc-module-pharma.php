...
class SSC_Module_Pharma extends SSC_Module {
                ...
                return array(
                        'reporter' => array(
                                ...
                                'fields'   => array(
                                        ...
                                        'name'    => array( 'label' => __( 'Reporter name', 'smart-support-chatbot' ), 'type' => 'text', 'required' => true ),
                                        'phone'   => array( 'label' => __( 'Contact phone', 'smart-support-chatbot' ), 'type' => 'tel', 'required' => true ),
                                        'patient_age'   => array( 'label' => __( 'Patient age (years)', 'smart-support-chatbot' ), 'type' => 'number' ),
                                        'patient_sex'   => array(
                        ...
                        'product' => array(
                                ...
                                'fields' => array(
                                        'product'       => array( 'label' => __( 'Product name', 'smart-support-chatbot' ), 'type' => 'product', 'required' => true ),
                                        'batch_number'  => array( 'label' => __( 'Batch / lot number', 'smart-support-chatbot' ), 'type' => 'text' ),
                                        'dose'          => array( 'label' => __( 'Dose and frequency used', 'smart-support-chatbot' ), 'type' => 'text' ),
                                        'route'         => array(
                        ...
                        'reaction' => array(
                                ...
                                'fields' => array(
                                        'description'  => array( 'label' => __( 'What happened? Describe the reaction and dates.', 'smart-support-chatbot' ), 'type' => 'textarea', 'required' => true ),
                                        'severity'     => array(
                                        ...
                                        'concomitant_drugs' => array( 'label' => __( 'Other medicines taken at the same time', 'smart-support-chatbot' ), 'type' => 'textarea' ),
                                ),
                        ),
                );
        }
                                ...
                                if ( isset( $field['options'] ) ) {
                                        if ( 'checkboxes' === $field['type'] ) {
                                                ...
                                                foreach ( $field['options'] as $value => $label ) {
                                                        $entry['options'][] = array( 'value' => $value, 'label' => $label );
                                                }
                                        } else {
                                                $entry['options'] = array();
                                                foreach ( $field['options'] as $value ) {
                                                        $entry['options'][] = array( 'value' => $value, 'label' => self::option_label( $key, $value ) );
                                                }
                                        }
                                }
                                $out[] = $entry;
        ...
        public function handle_submission( $params, $ip = '' ) {
                ...
                if ( ! empty( $params['ssc_hp'] ) ) {
                        return array( 'ok' => true, 'fake' => true ); // Honeypot.
                }
        ...
        public function handle_actions() {
                ...
                if ( current_user_can( 'manage_options' ) && isset( $_POST['ssc_pharma_setup_save'] ) && check_admin_referer( 'ssc_pharma_setup' ) ) {
                        ...
                        if ( ! is_email( $pv_email ) ) { wp_die( esc_html__( 'Enter a valid pharmacovigilance contact email.', 'smart-support-chatbot' ) ); }
                        update_option(
                                'ssc_pharma_setup',
                                array(
                                        'done'       => 1,
                        ...
                        wp_safe_redirect( add_query_arg( array( 'page' => 'ssc-pharma', 'configured' => 1 ), admin_url( 'admin.php' ) ) );
                        exit;
                }
                ...
                if ( isset( $_POST['case_id'] ) ) {
                        ...
                        if ( 'pharma_adr' !== $case_type ) { return; }
                }
                ...
                if ( isset( $_POST['ssc_case_status'], $_POST['case_id'] ) && check_admin_referer( 'ssc_case_' . (int) $_POST['case_id'] ) ) {
                        ...
                        wp_safe_redirect( add_query_arg( array( 'page' => 'ssc-pharma', 'updated' => 1 ), admin_url( 'admin.php' ) ) );
                        exit;
                }
                ...
                if ( isset( $_POST['ssc_case_privacy'], $_POST['case_id'] ) && check_admin_referer( 'ssc_case_privacy_' . (int) $_POST['case_id'] ) ) {
                        ...
                        if ( 'anonymize' === $action ) {
                                ...
                                wp_safe_redirect( add_query_arg( array( 'page' => 'ssc-pharma', 'view' => $id, 'privacy' => 'anon' ), admin_url( 'admin.php' ) ) );
                                exit;
                        }
                        if ( 'purge' === $action ) {
                                $this->purge_case( $id, $actor );
                                wp_safe_redirect( add_query_arg( array( 'page' => 'ssc-pharma', 'purged' => 1 ), admin_url( 'admin.php' ) ) );
                                exit;
                        }
                }
        ...
        protected function export_csv() {
                ...
                $page = 1;
                do {
                        $result = SSC_Schema::get_submissions( array( 'type' => 'pharma_adr', 'per_page' => 500, 'page' => $page ) );
