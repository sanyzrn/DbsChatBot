...
class SSC_REST {
        ...
        public function test_connection( $request ) {
                ...
                if ( null === $provider ) {
                        return rest_ensure_response( array( 'ok' => false, 'message' => __( 'Unknown provider.', 'smart-support-chatbot' ) ) );
                }
                $overrides = array();
         ...
         * @return WP_REST_Response
         */
        public function test_identity( $request ) {
                $provider = SSC_Providers::current();
                if ( null === $provider ) {
                        return rest_ensure_response( array( 'ok' => false, 'message' => __( 'Configure and verify an AI provider first.', 'smart-support-chatbot' ) ) );
                }
                $business = SSC_Settings::business();
                if ( '' === trim( (string) $business['org_name'] ) ) {
                        return rest_ensure_response( array( 'ok' => false, 'message' => __( 'Complete the business profile (step 1) first.', 'smart-support-chatbot' ) ) );
                }
                ...
                $result   = $provider->generate_with(
                        ...
                        $system,
                        array( array( 'role' => 'user', 'content' => $question ) ),
                        array( 'endpoint' => $provider->saved_credentials()['endpoint'], 'max_tokens' => 300, 'temperature' => 0 )
                );
