<?php
/**
 * The admin-ajax bridge (backward compatibility).
 *
...
class SSC_Ajax {
	...
	protected function post( $key, $textarea = false ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized on the next line; public endpoints are rate-limited by design.
		$value = wp_unslash( $_POST[ $key ] );
		return $textarea ? sanitize_textarea_field( (string) $value ) : sanitize_text_field( (string) $value );
	}

	/**
	 * Shared gate for every public action: size cap plus the same optional
	 * nonce policy the REST controller applies. Terminates the request on
	 * rejection, so callers can treat a return as success.
	 */
